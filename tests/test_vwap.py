...
def test_reversion_score():
    ...
    # Price=120. Z = (120-100)/10 = 2.0. Score = -2.0
    res = vwap.compute_vwap_features([], datetime.now(), "BTC", "sess", 120.0)
    assert res["vwap_mean_reversion_score"] == -2.0
    
    # Price=80. Z = (80-100)/10 = -2.0. Score = 2.0
    res2 = vwap.compute_vwap_features([], datetime.now(), "BTC", "sess", 80.0)
    assert res2["vwap_mean_reversion_score"] == 2.0
