...
def test_vwap_constant_price():
    ...
    # Price always 100
    vwap_result = vwap.compute_vwap_features(trades, datetime.now(), "BTC", "sess", 100.0)
    
    # No deviation
    assert vwap_result["session_vwap_distance"] == 0.0
    assert vwap_result["global_vwap_distance"] == 0.0
    assert vwap_result["vwap_band_flag_1sd"] is False
...
def test_vwap_distances():
    ...
    vwap_result = vwap.compute_vwap_features([], datetime.now(), "BTC", "sess", 105.0)
    
    # Dist = (105 - 100)/100 = 0.05
    assert abs(vwap_result["session_vwap_distance"] - 0.05) < 1e-9
...
def test_reversion_score():
    ...
    vwap_result = vwap.compute_vwap_features([], datetime.now(), "BTC", "sess", 120.0)
    assert vwap_result["vwap_mean_reversion_score"] == pytest.approx(-66.67, abs=1.0)

    # Price=80. Z = (80-100)/10 = -2.0.
    # Score magnitude = 2.0/3.0 * 100 = 66.67
    # Price < VWAP so score is positive (mean reversion would push up)
    vwap_result_second = vwap.compute_vwap_features([], datetime.now(), "BTC", "sess", 80.0)
    assert vwap_result_second["vwap_mean_reversion_score"] == pytest.approx(66.67, abs=1.0)
